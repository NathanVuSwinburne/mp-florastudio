  ...
  return (
    <Card className="group relative overflow-hidden p-5 transition-shadow hover:shadow-[var(--shadow-bloom)]">
      ...
      <div className="absolute right-4 top-4 flex items-center gap-1 rounded-full bg-blush-100 px-2.5 py-1 text-xs font-semibold text-rose-600">
        <Gem className="h-3 w-3" /> Lv {plant.level}
      </div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1 pt-1">
          ...
          <Badge variant={status} className="mt-2">{STATUS_LABEL[status]}</Badge>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-blush-50/70 p-2.5">
          <p className="flex items-center gap-1 text-[0.65rem] uppercase tracking-wide text-plum-400">
            <Heart className="h-3 w-3 text-rose-400" /> Health
          </p>
          <p className="font-display text-2xl text-plum-900">{plant.health}%</p>
        </div>
        <div className="rounded-xl bg-blush-50/70 p-2.5">
          <p className="flex items-center gap-1 text-[0.65rem] uppercase tracking-wide text-plum-400">
            <TrendingUp className="h-3 w-3 text-sage" /> Value
          </p>
      ...
      <div className="mt-3">
        <div className="mb-1 flex justify-between text-[0.65rem] text-plum-400">
          <span>XP to Lv {plant.level + 1}</span>
      ...
      <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2">
        <StatBar label="Water" value={plant.stats.hydration} tone="hydration" showValue={false} />
        <StatBar label="Sun" value={plant.stats.sunlight} tone="sunlight" showValue={false} />
        <StatBar label="Feed" value={plant.stats.fertiliser} tone="fertiliser" showValue={false} />
        <StatBar label="Growth" value={plant.stats.growth} tone="growth" showValue={false} />
      </div>
